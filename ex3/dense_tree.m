...
get_next=@(root,l)[root(1)+l/3*sin(root(3)) root(2)+l/3*cos(root(3)) root(3)+dtheta;
    ...
    root(1)+l/3*sin(root(3)) root(2)+l/3*cos(root(3)) root(3);
    root(1)+l*2/3*sin(root(3)) root(2)+l*2/3*cos(root(3)) root(3);
    root(1,:)];
...
hold off
title("dense tree("+k+"); angle:"+dtheta*180/pi+"°; scale:"+scale)
axis equal
end